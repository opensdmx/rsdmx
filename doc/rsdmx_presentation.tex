...
\section{Architecture of rsdmx}
  ...
  \begin{frame}
  	\frametitle{rsdmx}
  	\framesubtitle{A package for reading SDMX data and metadata documents in R}
  	The rsdmx package:
  	\begin{itemize}
		\item provides a set of tools to read SDMX-ML data and metadata documents
		\item focuses on the SDMX-ML format (main SDMX format)
		\item provides a single end-user \texttt{readSDMX} function to read any SDMX-ML document, whatever its type
		\item provides a set of methods to convert SDMX data into tabular data (\texttt{data.frame})
  	\end{itemize}
  \end{frame}
...
\end{rsdmx}
  ...
  \begin{frame}
  	...
  	\begin{itemize}
  		\item download the SDMX-ML document (done by default, unless \texttt{fromURL} argument is set to \texttt{TRUE})
  ...
  \begin{frame}
  	...
  	2 different parsing technics:
  		\begin{itemize}
  			...
  			\end{itemize}
  			\vspace{0.1cm}
  			\item Alternative approach (under investigation): using the \textbf{Simple API for XML (SAX)}
  			\begin{itemize}
...
\begin{Sinput}
R> require(devtools)
R> install_github("rsdmx", "opensdmx")
\end{Sinput}
\end{Schunk}
  	\end{itemize}
...
Read a SDMX generic dataset in \R
...
\section{Conclusions \& Perspectives}
  ...
  %rsdmx package - Success stories %
  %--------------------------------%
  \begin{frame}
  	\frametitle{rsdmx}
  	\framesubtitle{Success stories}
  	...
  	\begin{itemize}
    	...
    	\item with \textbf{national} data sources: \href{http://stat.abs.gov.au/}{Australian Bureau of Statistics (ABS)}, \href{https://www.ons.gov.uk/ons/apiservice/web/apiservice/home}{UK's Office of National Statistics (ONS)}, \href{http://www.bundesbank.de/}{Deutsche Bundesbank}
  	\end{itemize}
  	
  	Used in web projects, such as:
  	\begin{itemize}
  ...
  \begin{frame}
  	...
  	\begin{itemize}
  		\item enable the \textbf{Simple API for XML (SAX)} parsing technic for large datasets
  ...
  \begin{frame}
  	...
  	\texttt{rsdmx} can play a \textbf{fundamental role} for \textbf{exploiting} and \textbf{co-analyzing statistics} from scattered data sources by means of a \textbf{simple \R tool}.
  	
  	Until now, \texttt{rsdmx} was born from a \textbf{voluntary initiative}, and is now a \textbf{published library} with a \textbf{growing number of users}. To guarantee the sustainability of \texttt{rsdmx}, we are seeking for \href{https://github.com/opensdmx/rsdmx/wiki\#package_development_funding}{\textbf{fundings}} to:
  	\begin{itemize}
		\item \textbf{implement, validate and release improvements}, especially core enhancements (e.g. support for SAX XML Parser)
  ...
  \begin{frame}
  	...
  	\begin{itemize}
		...
		\begin{itemize}
			...
			\vspace{0.1cm}
			\item wiki page: \href{https://github.com/opensdmx/rsdmx/wiki}{https://github.com/opensdmx/rsdmx/wiki}
			\vspace{0.1cm}
		\end{itemize}
		\item mailing lists:
		\begin{itemize}		
			\item \href{https://groups.google.com/forum/\#!forum/rsdmx}{rsdmx}
			\item \href{https://groups.google.com/forum/\#!forum/rsdmx-dev}{rsdmx-dev}
		\end{itemize}				
		\vspace{0.1cm}
		\item on the Comprehensive R Archive Network (CRAN): \href{http://cran.r-project.org/web/packages/rsdmx}{http://cran.r-project.org/web/package/rsdmx}
  	\end{itemize}  	
  \end{frame}  
